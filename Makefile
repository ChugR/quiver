...
QPID_MESSAGING_CPP_ENABLED := \
	$(shell PYTHONPATH=python:$PYTHONPATH scripts/check-cpp-header "qpid/messaging/Message.h" 1> /dev/null 2>&1 && echo yes || echo no)
QPID_MESSAGING_PYTHON_ENABLED := \
	$(shell PYTHONPATH=python:$PYTHONPATH scripts/check-python2-import "qpid_messaging" 1> /dev/null 2>&1 && echo yes || echo no)
QPID_PROTON_C_ENABLED := \
	$(shell PYTHONPATH=python:$PYTHONPATH scripts/check-cpp-header "proton/proactor.h" 1> /dev/null 2>&1 && echo yes || echo no)
QPID_PROTON_CPP_ENABLED := \
	$(shell PYTHONPATH=python:$PYTHONPATH scripts/check-cpp-header "proton/message.hpp" 1> /dev/null 2>&1 && echo yes || echo no)
QPID_PROTON_PYTHON_ENABLED := \
	$(shell PYTHONPATH=python:$PYTHONPATH scripts/check-python3-import "proton" 1> /dev/null 2>&1 && echo yes || echo no)
