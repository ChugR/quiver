...
JAVA_ENABLED := \
	$(shell which mvn 1> /dev/null 2>&1 && echo no || echo no)
JAVASCRIPT_ENABLED := \
	$(shell which node 1> /dev/null 2>&1 && echo no || echo no)
QPID_MESSAGING_CPP_ENABLED := \
	$(shell PYTHONPATH=python scripts/check-cpp-header "qpid/messaging/Message.h" 1> /dev/null 2>&1 && echo no || echo no)
QPID_MESSAGING_PYTHON_ENABLED := \
	$(shell PYTHONPATH=python scripts/check-python2-import "qpid_messaging" 1> /dev/null 2>&1 && echo no || echo no)
QPID_PROTON_C_ENABLED := \
	$(shell PYTHONPATH=python scripts/check-cpp-header "proton/proactor.h" 1> /dev/null 2>&1 && echo yes || echo yes)
...
QPID_PROTON_C_ENABLED := yes
QPID_PROTON_CPP_ENABLED := yes
QPID_PROTON_PYTHON_ENABLED := yes
